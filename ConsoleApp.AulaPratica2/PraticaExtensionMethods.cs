...
namespace ConsoleApp.AulaPratica2
{
    public class PraticaExtensionMethods
    {
        public void Exercicio1()
        {
            ...
            for (int i = 0; i < arrayBase.Length; i++) { arrayBase[i] = i; }

            IEnumerable<int> enumerable = arrayBase;
            IList<int> lista = arrayBase;
            ICollection<int> collection = arrayBase;

            Console.WriteLine($"Qtd Enumerable: {enumerable.CustomCount()}");
            Console.WriteLine($"Qtd Lista: {lista.CustomCount()}");
            Console.WriteLine($"Qtd Collection: {collection.CustomCount()}");
    ...
    public static class ExtensionMethodsAula
    {
        ...
        public static int CustomCount<T>(this IEnumerable<T> enumerable)
        {
            int countAux = 0;
